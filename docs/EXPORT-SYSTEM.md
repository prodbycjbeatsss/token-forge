...
Create → Generate → Edit → System Validation → Export Format Selection → Export Readiness Validation → Export
...
TokenForge Token Model ↓ Export Adapter ↓ Target Format
...
Select format ↓ Complete System is targeted ↓ Readiness Check ↓ Export

--------------------------------------------------------------------------------

5. Export Workflow
The V1 export workflow is standardized as:
Create → Generate → Edit → System Validation → Export Format Selection → Export Readiness Validation → Export
...
CSS selected ↓ CSS Readiness ↓ PASS
User changes format
JavaScript / TypeScript selected ↓ JavaScript / TypeScript Readiness ↓ New evaluation
...
Current Project State ↓ Current Export Selection ↓ Final Readiness Check ↓ ┌────┴────┐ PASS      FAIL ↓          ↓ Export     Return to correction
...
Internal Token Model ↓ Export Adapter ↙     ↓      ↘ CSS    JSON    Tailwind v4 ... JavaScript / TypeScript
...
The process is:
Internal Token System ↓ General System Validation ↓ DTCG selected ↓ DTCG Readiness Validation ↓ DTCG-specific compatibility checks ↓ Safe deterministic transformation ↓ DTCG-compatible JSON
...
:root { --color-primary: ...; --spacing-md: ...; --radius-md: ...; }
...
A conceptual result is:
export const tokens = { color: { primary: "...", }, };
...
The process is:
TokenForge name ↓ Target compatibility check ↓ Safe deterministic transformation possible? ↙       ↘ Yes        No ↓          ↓ Export     Error
...
The process is:
Unsupported representation ↓ Export Readiness Error ↓ User chooses how to resolve it
...
TokenForge token ↓ CSS transformation ↓ CSS output
...
For example:
TokenForge System ├── JSON ├── CSS ├── Tailwind v4 CSS └── JavaScript / TypeScript
...
Validation ↓ Is the system valid? ↓ Export System ↓ Can the selected representation be produced?
...
DTCG-compatible JSON CSS Tailwind v4 CSS JavaScript / TypeScript
...
The architecture is:
Canonical Token System ↓ System Validation ↓ Export Selection ↓ Format Readiness Validation ↓ Export Adapter ↓ Target Format
