...
The Validation Engine explicitly evaluates these calculated metrics across:
1.  **Text against background:** Checking 4.5:1 (normal) or 3:1 (large text).
2.  **UI component boundaries against background:** Checking 3:1 for interactive elements (e.g., input borders).
3.  **Interactive state differentiation:** Ensuring visual changes (e.g., Hover vs. Default) satisfy contrast distinction requirements without relying solely on colour.
...
When a user overrides a specific semantic or primitive token, the override pins that exact value. It does not sever the underlying scale generation or dependency graph unless the user explicitly toggles an 'unlink' or 'detach' control. 
...
AI ↓ Recommendation / explanation ↓ Deterministic Colour Engine ↓ Validation ↓ Result
